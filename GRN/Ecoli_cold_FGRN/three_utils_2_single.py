...
def transform_data_single(train_data):
    ...
    dataX_tf = torch.tensor(featuretf, dtype=torch.float32)[:, np.newaxis, :]
    dataX_target = torch.tensor(featuretarget, dtype=torch.float32)[:, np.newaxis, :]
    print("the shape of dataX_tf: ", dataX_tf.shape)
    print("the shape of dataX_target: ", dataX_target.shape)

    label_ = np.array(label_)
    labelY = torch.tensor(label_, dtype=torch.int64)

    # Convert label_ to one-hot tensors
    num_classes = len(np.unique(label_))
    one_hot_labels = torch.nn.functional.one_hot(labelY, num_classes=num_classes)

    position = torch.tensor(position)

    return dataX_tf, dataX_target, one_hot_labels, position
...
def transform_data_single_net(train_data):
    ...
    return dataX_tf, dataX_target, net_tf_s, net_tf_t, net_target_s, net_target_t, one_hot_labels, position
