...
# undirected edges to directed edges
    if task in [1,2,3,4]:
        print(f"Initial train edges shape: {train_edges.shape}")
        pos_train_edges, _ = undirected_label2directed_label(A, train_edges, 'train_di', pn_ratio)
        print(f"Positive train edges shape after conversion: {pos_train_edges.shape}")
        ...
        print(f'shape of sourcenode{pos_train_edges[:,0].shape}and targetnode{pos_train_edges[:,1].shape}')
        train_eid = graph.edge_ids(pos_train_edges[:,0], pos_train_edges[:,1])
        # train_eid = graph.edge_ids(pos_train_edges[:,0].long(), pos_train_edges[:,1].long())
...
def load_training_data(args):
    ...
    whole_graph.ndata['h'] = torch.tensor(embedding_matrix, dtype=torch.float32)
    ...
    train_edges = np.loadtxt(training_file, dtype=np.int64, delimiter=',')
    ...
    whole_graph.edata['label'] = torch.full((whole_graph.num_edges(),), -1, dtype=torch.int64)
    whole_graph.edata['label'][pos_eid_train] = true_train_edges[:, 2]
    whole_graph.edata['label'][rev_eid_train] = rev_train_edges[:, 2]

    whole_graph.edata['exist'] = torch.ones(whole_graph.num_edges(), dtype=torch.float32)
    ...
    whole_graph.edata['am_exist'] = torch.zeros(whole_graph.num_edges(), dtype=torch.float32)
    ...
    _, idx = torch.unique(w_edges, dim=0, return_inverse=True)
    ...
    return dataloader, whole_graph, torch.tensor(embedding_matrix, dtype=torch.float32)
...
def load_testing_data(args, task, whole_graph):
    ...
    if os.path.exists(testing_file):
        new_test_edges = torch.tensor(np.loadtxt(testing_file, dtype=np.int64, delimiter=',')).to(args.device)
    else:
        print('no testing file')
        new_test_edges = torch.tensor([], dtype=torch.int64).to(args.device)
    ...
    if os.path.exists(val_file):
        new_val_edges = torch.tensor(np.loadtxt(val_file, dtype=np.int64, delimiter=',')).to(args.device)
    else:
        print('no validation file')
        new_val_edges = torch.tensor([], dtype=torch.int64).to(args.device)
